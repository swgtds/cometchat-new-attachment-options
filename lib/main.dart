            ...
            child: PageView(
              ...
              children: [
                CometChatConversations(
                  ...
                  onItemTap: (conversation) async {
                    final appEntity = conversation.conversationWith;
                    final isGroup =
                        conversation.conversationType ==
                        CometChatConversationType.group;

                    if (isGroup && appEntity is Group) {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) =>
                              MessagesScreen(group: appEntity),
                        ),
                      );
                    } else if (!isGroup && appEntity is User) {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => MessagesScreen(user: appEntity),
                        ),
                      );
                    } else {
                      debugPrint(
                        "❌ Unable to identify user or group. Entity: $appEntity",
                      );
                    }
                  },
                ),
